...
data.results <- setnames(setcolorder(data.to.table[data.knn][order(type)],c('Datasets','type','baseline','nbr','nbr.disc','nbr.disc.unsup','knn','knn.disc','knn.disc.unsup'))[,-c(5,8),with=F],c(4:7),c('nbr.cont','nbr.disc','knn.cont','knn.disc'))
@
...
The paper is organized as follows: \vref{ch2-sec:learning} provides the formalization of the label ranking problem; \vref{ch2-sec:naivebayes} briefly  describes the naive Bayes algorithm for classification; \vref{ch2-sec:adapting} shows the adaptation of the NB algorithm for label ranking (NB4LR); \vref{ch2-sec:cases} provides some extensions of NB4LR; namely, the scenario of features having a continuous values (\vref*{ch2-sec:nbr.cont})  and a case when rankings are part of time series (\vref*{ch2-sec:time}); \vref{ch2-sec:data} outlines the datasets used for the experiments;
%section \vref{ch2-sec:metalearning} explains  the problem of metalearning, which will be the application domain for the empirical evaluation;
\vref{ch2-sec:results} presents empirical results; finally, \vref{ch2-sec:conclusion} concludes with the goals for future work.
...
Using \vref{ch2-eq04}, we can rewrite \vref{ch2-eq01} as
\begin{align}
\label{ch2-eq02}
...
\lambda)=\prod_{j=1}^m P(x_{i,j}|\lambda)$. Substituting this expression into \vref{ch2-eq02}, we obtain the naive Bayes classifier:
\begin{equation}
 \label{ch2-eq03}
...
Consider the classic problem of the ``play/no play" tennis based on the weather conditions. The naive Bayes classification algorithm can be successfully applied to this problem~\citep[chap. 6]{mitchell1997}. For illustration purposes, we extend this example application to the label ranking setting by replacing the target with a ranking on the preferences of a golf player regarding three tennis courts on different days (\vref*{ch2-tab01}).

The last three columns in \vref{ch2-tab01} represent the ranks of the tennis courts A, B and C.
...
The adaptation should take into account the differences in nature between label rankings and classes. For example, if we consider label ranking as a classification problem, then the prior probability of ranking $\{A,B,C\}$ on the data given in \vref{ch2-tab01} is $P(\{A,B,C\})$ = \Sexpr{length(which(data.table(data)[,paste0(a,b,c)==paste0(1,2,3)]))}/\Sexpr{nrow(data)} = \Sexpr{round(length(which(data.table(data)[,paste0(a,b,c)==paste0(1,2,3)]))/nrow(data),2)}, which is quite high. On the other hand, the probability of $\{A,C,B\}$ is quite low, $P(\{A,C,B\})$ = \Sexpr{length(which(data.table(data)[,paste0(a,b,c)==paste0(1,3,2)]))}/\Sexpr{nrow(data)} = \Sexpr{round(length(which(data.table(data)[,paste0(a,b,c)==paste0(1,3,2)]))/nrow(data),2)}. However, taking into account the stochastic nature of these rankings~\citep{cheng2009}, it is intuitively clear that the observation of $\{ A,B,C\}$ increases the probability of observing $\{A,C,B\}$ and vice-versa. This affects even rankings that are not observed in the available data. For example, the case of unobserved ranking $\{B,A,C\}$ in \vref{ch2-tab01} would not be entirely unexpected in the future considering a similar observed ranking $\{B,C,A\}$. %$P(\{C,A,B\}=2/6=0.33$.
...
We say that the prior probability is the mean of similarity of a given rankings to all the others. We measure similarity  using the Spearman correlation coefficient (\vref*{ch2-eq00}). \Vref{ch2-eq:prior} shows the average similarity of one ranking relative to others. The greater the similarity between two particular rankings, the higher is the probability that the next unobserved  ranking will be similar to the known ranking. Take a look at \vref{ch2-tab02} with the calculated prior probability for the unique rankings. We also added a column with prior probabilities considering the rankings as one class.
...
Applying \vref{ch2-eq03}, we get the estimated posterior probability of ranking $y$:
\begin{align}
...
As for the conditional label ranking probability, the equation for the weighted mean (\vref*{ch2-cont}) becomes:
\begin{equation}
\label{ch2-mu.w}
...
The performance of the label ranking methods was estimated using a methodology that has been used previously for this purpose~\citep{brazdil2003}. It is based on 10-fold cross validation. The accuracy of the rankings predicted by methods was evaluated by comparing them to the target rankings (i.e., the rankings based on the observed performance of the algorithms) using the Spearman's correlation coefficient (\vref*{ch2-eq00}). The code for all the examples in this paper has been written in R~\citep{rdl08}.



%\subsection{Results}

The results of the experiments are presented in \vref{ch2-results}. We report that the naive Bayes for label ranking for continuous case exhibits the maximum number of datasets for which it out-performed the baseline and is competitive with the state-of-the-art. The bold numbers in the table represent the value of the average Spearman correlation across 10-folds that are higher than that of the baseline.

For the continuous case, the naive Bayes for label ranking algorithm outperformed the baselines in \Sexpr{length(data.table(t(kebi.results.nbr))[,which(baseline<=nbr)])} datasets out of \Sexpr{nrow(data.to.table)}. For the discretized case the number of outperformed datasets is \Sexpr{length(data.table(t(kebi.results.nbr))[,which(baseline<nbr.disc.unsup)])}. The state-of-the-art algorithm outperformed the baseline in continuous and discrete scenarios in \Sexpr{length(data.table(t(kebi.results.knn))[,which(baseline<knn)])} and \Sexpr{length(data.table(t(kebi.results.knn))[,which(baseline<knn.disc.unsup)])} respectively. Observe, that given the different nature of datasets (types A or B) and different variable classes (continuous vs. nominal) both label ranking algorithms exhibit relatively great performance in predicting the rankings.
