...
Consider the classic problem of the ``play/no play" tennis based on the weather conditions. The naive Bayes classification algorithm can be successfully applied to this problem~\citep[chap. 6]{mitchell1997}. For illustration purposes, we extend this example application to the label ranking setting by replacing the target with a ranking on the preferences of a golf player regarding three tennis courts on different days (\ref*{ch2-tab01}).

The last three columns in \ref{ch2-tab01} represent the ranks of the tennis courts A, B and C.
