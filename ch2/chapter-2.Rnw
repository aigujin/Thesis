...
setwd('~/Documents/PhD/Projects/Nbr/')
library(ProjectTemplate)
load.project()
source('~/Documents/PhD/Projects/MetaRanks/my.paths.R')
...
data <- read.csv("~/Documents/PhD/Projects/Naive.Bayes.separate.functions/table.paper.csv", header = T, sep=",")
...
\label{ch2-tab02}
\end{table}
As stated above, the ranking $\{A,C,B\}$, due to its  similarity to the other two rankings, achieves a higher probability (\Sexpr{prob.table[4,5]})\footnote{Since we measure $P_{LR}$ as a similarity between rankings, it would not sum to one as the in case of probability for classification.}.
