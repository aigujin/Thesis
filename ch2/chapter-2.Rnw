...
The description of individual data set presented in \vref{ch2-data.descr}. A type of a data set is described in~\cite{cheng2009}:
\begin{quotation}
...
\end{center}
\end{table}
The results of the experiments are presented in \vref{ch2-results}. We report that the naive Bayes for label ranking for continuous case exhibits the maximum number of datasets for which it out-performed the baseline. The bold numbers in the table represent the value of the average Spearman correlation across 10-folds that are higher than that of the baseline.
