...
setkey(metric.vvs,Stock)
data.to.display <- c('Variable','method','Stock','nbr.val','mean', 'median','std.dev')
desc.ind <- setnames(na.omit(metric.vvs[stocks])[,list(length(unique(Stock)),.N,mean(value,na.rm=T),median(value,na.rm=T),sd(value,na.rm=T)),by=.(vvs,method)],data.to.display)
...
The predicition is done using the state variables that affect the ranking the most. The results from \vref{ch3-sec:results} suggests that these are \emph{disp}, \emph{infl}, \emph{t.bill} from analyst-specifice and macroecomic variable types. Further, we predict the ranking based on the 16 quarter sliding window sample, i.e., to predict the rankings for the 16\textsuperscript{th} quarter, we use data from the previous 15. This is done to discard the very old information and we assume that 4 years is sufficient enought to capture the analysts' historical preformance.
...
Panel B of the table demonstrates the results of trading with consensus in among analysts about price target. The annulized cumulative return of this strategy is \Sexpr{round(bl.results['true',dom.str,1,con],2)}\% and the Sharpe ration is  \Sexpr{round(bl.results['true',dom.str,3,con],3)}. This shows that the consensus strategy showed below the \market{} performance.
