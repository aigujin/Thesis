...
#desc.ind <- setnames(na.omit(pt.dp)[,list(length(unique(Stock)),.N,mean(value,na.rm=T),median(value,na.rm=T),sd(value,na.rm=T)),by=.(vvs,method)],data.to.display)

#stat.all.vvs <- acast(melt(desc.ind,id.vars=c('Variable','method')),Variable~method~variable,value.var='value')
share.vvs <- acast(na.omit(eps.dp)[round(metric)>0][,mean(metric),by=.(vvs,method)][,share:=V1/sum(V1)*100,by=.(method)][,mean(share,na.rm=T),by=.(vvs,method)],vvs~method,value.var='V1')
...
eps.rank.results <- cbind(eps.rank[,.(year,true)],eps.rank[,lapply(.SD,function(i){ifelse(naive<i&default<i,paste0('\\textbf{',round(i,4),'}'),round(i,4)) }),.SDcols=c('naive','default','last','diff','random','roll.sd')])

### t-stat:
#test <- dcast.data.table(eps.accu,q.id+Stock~variable)

#test[,lapply(.SD,function(i){t.test(naive,i)$p.value}),.SDcols=c('naive','default','last','diff','random','roll.sd'),by=.(q.id)]
...
Rankings of  analysts have potential benefit for investors only if one can predict these rankings in advance (see \vref{ch1}). In this chapter we apply the methodologies developed above to predict the rankings of financial analysts and use these rankings to build trading strategies.

Given the results form \vref{ch1-sec:results}, we base our rankings on analysts' target prices as it has been shown that strategies based on these rankings are the ones that yield the highest cumulative annualized return (\vref{tab:strategy}).
...
%'
%'
%' We follow the Black-Litterman procedure developed in~\cite{aiguzhinov2013a}. We repeat here the \vref{ch4-fig:bl} to outline the main steps in trading:
%' \begin{enumerate}
...
%' \item Based on these  predicted rankings and analysts' price targets,  we define $Q_{q,s}$ and $\Omega_{q,s}$ (see (\vref*{ch4-eq:q})  and (\vref*{ch4-eq:omega}));
...
%' \end{equation}
%' where $\rho$ is the Spearman correlation of the predicted rankings (\vref*{ch4-sec:lr}).
%'
%'
...
%' \end{table}
%'
%' % Figure (\vref{ch4-fig:tr-pmafe}) plots the relation between the actual ranking and the forecast errors (PMAFE). We convert absolute rankings into relative terms by applying:
%' % \begin{equation}
...
%'
%' $P_{LR}(x_{i,a}|y)$ in \vref{ch4-eq:nb} is the conditional label ranking probability of a variable $x_i$ of attribute $a$, ($v_{i,a}$):
%' \begin{equation}
...
%'
%' As for the conditional label ranking probability, the equation for the weighted mean (\vref*{ch4-cont}) becomes:
%' \begin{equation}
%' \label{ch4-mu.w}
...
%'
%' \Vref{ch4-eq:disp} calculates the dispersion among the analysts which is a variance of price targets of all analysts for a given stock. \Vref{ch4-eq:uncert} defines the Uncertainty component of the dispersion per~\cite{barron2009}. As we observe, it is the sum of squared mean errors and dispersion. \Vref{ch4-eq:assym} is the proxy for information asymmetry which a function of dispersion, squared mean error, and a number of price targets.
%'
%'
...
We apply the ranking prediction algorithm developed in \vref{ch2} on EPS rankings outlined in \vref{ch1}. Further, we selected state variables consistent with \vref{ch3}, i.e., we use variable that have more than 10\% contribution to the rankings. \Vref{ch4:tab-dp} demonstrates the total discriminative power of state variables for different states for EPS ranking. The variables that contribute the most to the rankings are:
...
\ The table demonstrates the contributions of each of the state variable to the analysts' rankings for different states of the world.
...
We report the ranking prediction results in \vref{ch4:tab-rank}. The table presents the average across stocks Spearman rank correlation between the \true{} and all other ranking methods. We report that the label ranking algorithm is able to outperform the both baselines (\naive{} and \default{}) in \diff{} state of the world. \Vref{ch4:fig-accur} depicts the smoothed series of the average Spearman correlations. We applied the local polynomial regression fitting~\citep{cleveland1992} to smooth the accuracy for better presentation.

Looking at baselines rankings, i.e., rankings that obtained without label ranking model, we observe that for the first half of the sample period (from 2000 until 2004) the \naive{} rankings  demonstrate a constant increase in average ranking accuracy. This means that during this period, rankings of the analysts, on average, did not change from one quarter to another. Starting from 2004, the average accuracy of the \naive{} rankings begins to decrease until the end of the sample period.
...
As far as the predicted rankings are concerned, observe that the \diff{} average accuracy has been constantly above the other methods and only during the period of crisis it drop significantly. In fact, except the \random{}, all predicted methods reach the maximum average accuracy at around 2007, a pre-crisis time, and all, except the \default{}, showed a downward trend afterwards.

The results from the predicting of the rankings based on the EPS forecasts reveal that the average accuracy of the predicted rankings that outperforms the baselines achieved when the independent variables that contributed to the rankings more than 10\% exhibit a stationary behavior (via first-difference method).



\begin{table}
\caption{Ranking accuracy over time}
\ The table presents ranking accuracy of predicted EPS rankings compared to baselines. Bold are the results that outperform both the baselines.
...
print(xtable(eps.rank.results,digits=4),only.contents=T,include.colnames=F,include.rownames=F,hline.after=NULL,add.to.row=list(pos=list(10),command=c('\\midrule \n')),sanitize.text.function=function(x) x)
@
...
Panel B of the table demonstrates the results of trading with consensus in among analysts about price target. The annualized cumulative return of this strategy is \Sexpr{round(bl.results['true',dom.str,1,con],2)}\% and the Sharpe ration is  \Sexpr{round(bl.results['true',dom.str,3,con],3)}. This shows that the consensus strategy showed below the \market{} performance.
...
@
which is based on our algorithm of predicting the rankings. This strategy resulted in annual cumulative return of \Sexpr{max(bl.results[2:8,dom.str,1,tp])}\% and the Sharpe ratio of \Sexpr{max(bl.results[2:8,dom.str,3,tp])}. In addition, the average per quarter turnover ratio of this strategy of \Sexpr{max(bl.results[2:8,dom.str,5,tp])} implies relative low trading costs.


\vref{ch4-fig:bl-results} plots the graphical representation of the cumulative returns for all methods of trading strategy. We see that the \true{} strategy is always on top of all the others. We observe that the best outcome of the strategy achieved for the method of prediction of rankings with first difference of the independent variables.
...
%' \toprule
%'
%' Variable & Stock & Median & Mean & std.dev\\
%' \midrule
...
%'
%' print(xtable(stat.all.vvs[1:3,1,c(1,4,3,5)],display=c('s','d','f','f','f')),only.contents=T,include.colnames=F,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."))
%' @
%' \end{tabularx}
%'
%' \begin{tabularx}{\linewidth}{r*{5}{Y}}
...
%' print(xtable(stat.all.vvs[4:9,1,c(1,4,3,5)],display=c('s','d','f','f','f')),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."))
%' @
%' \end{tabularx}
%'
%' \begin{tabularx}{\linewidth}{r*{5}{Y}}
...
%' print(xtable(stat.all.vvs[10:13,1,c(1,4,3,5)],display=c('s','d','f','f','f')),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."))
%' @
%' \bottomrule
%' \end{tabularx}
%' \end{table}
...
%' \ Table \vref{ch4-tab:accuracy} depicts the average Spearman correlation (eq.\vref{ch4-eq:spearman}). We observe that \naive{} achieve the average correlation of \Sexpr{round(descr.pt.accuracy['naive','mean'],3)} (median=\Sexpr{round(descr.pt.accuracy['naive','median'],3)}). From the four different methods of label ranking the most successful is \rollsd{} with correlation of \Sexpr{round(descr.pt.accuracy['roll.sd','mean'],3)} (median=\Sexpr{round(descr.pt.accuracy['roll.sd','median'],3)}).
%' \begin{center}
%' \begin{tabular}{ccccc}
%' \toprule
...
ggplot(final.bl[confAgg==dom.str&Views!='EPS'],aes(x=as.Date(Quarters),y=cum.ret,group=Method,color=Method,shape=Method))+geom_line(size=0.5)+facet_wrap(~Views,scale='free_x')+ylab('Portfolio wealth (initial=$100)')+xlab('Quarters')+ggtitle('Portfolio performance with $100 initial investment')+theme_bw()+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),text=element_text(size=20,family='Times'))+geom_point()+geom_hline(yintercept=100)+scale_shape_manual(values=1:8)

#ggplot(final.bl[confAgg==dom.str&Views=='TP'&Method!='last'&Method!='random'&Method!='roll.sd'],aes(x=as.Date(Quarters),y=cum.ret,group=Method,color=Method,shape=Method))+geom_line(size=0.5)+facet_wrap(~Views,scale='free_x')+ylab('Portfolio wealth (initial=$100)')+xlab('Quarters')+ggtitle('Portfolio performance with $100 initial investment')+theme_bw()+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),text=element_text(size=20,family='Times'))+geom_point()+geom_hline(yintercept=100)+scale_shape_manual(values=1:8)
#ggsave(filename='forPresentation.pdf')
