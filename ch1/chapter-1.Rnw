...
The analysis of the sub-periods performance of the \textit{TP} and \textit{EPS} -based strategies depicted in Table \ref{tab:substrategy} shows that the latter outperforms the former in terms of the annualized cumulative return only for the first two periods: in  2000Q1/2004Q4  \Sexpr{periods.array[1,'EPS',3,2,1,1]}\% vs. \Sexpr{periods.array[1,'TP',3,2,1,1]}\%  in 2001Q1/2005Q4 \Sexpr{periods.array[2,'EPS',3,2,1,1]}\%  vs. \Sexpr{periods.array[2,'TP',3,2,1,1]}\% respectively for the \textit{EPS} and the \textit{TP} strategies. For the all the other sup-periods, the annualized cumulative returns of the \textit{EPS} strategy are lower than those of the \textit{TP} strategy.
