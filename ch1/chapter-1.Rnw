...
#core.dt <- na.omit(q.data)[,core.b:=.N>=12,by=list(Stock,analyst)][(core.b)][,rank:=rank(score),by=list(q.id,Stock)][,core.s:=.N>=3,by=list(q.id,Stock)][(core.s)]

#core.dt <- core.dt[,year:=format(q.id,'%Y')]
#pt.contin.dt <- unique(core.dt[,cont.rank:=.N>8,by=.(q.id,Stock)][(cont.rank)],by=c('q.id','analyst','Stock'))
...
The paper is organized as follows: \vref{ch1-sec:ranking} provides motivation to use rankings of financial analysts; \vref{ch1-sec:trading} outlines our proposed  trading strategies; \vref{ch1-sec:rankings} describes the sample and presents some preliminary results; \vref{ch1-sec:results} presents and discusses the results; and \vref{ch1-sec:conclusion} concludes.
...
We define a trading strategy as follows (\vref*{fig:bl}):
\begin{enumerate}
\item  At the beginning of quarter $t$ for each stock $i$,   we define $Q$ and $\Omega$ (see \vref{def-q} and \vref{def-omega});
...
The rank  that enters \vref{eq:weight} is average analyst's $PMAFE^{TP}$ over a particular quarter:
\begin{equation}
...
\end{equation}
where $D$ are the number of trading days in a quarter and $N$ is the number of analysts with a valid TP. \vref{fig:example} shows an example.
...
%\end{itemize}
\end{itemize}
where $Q_{t}$ is the analysts' expected rank-weighted stock return (\vref*{rankq})
...
Over the sample period, the total number of equity research firms\footnote{We use words ``analyst" and ``equity research firms" interchangeably.} in TP dataset is \Sexpr{q.data[,.N,by=.(Broker,type)][,.N,by=type][,N][2]}, covering \Sexpr{q.data[,.N,by=.(Stock,type)][,.N,by=type][,N][2]} S\&P500 stocks. Given the fact that financial analysts commonly issue TP with the one year horizon\footnote{According to Wharton Research Data Services (WRDS), 92.33\% of all price targets reported in I/B/E/S have a 12-month horizon~\citep{glushkov2009}.}, we assume that analysts keep their TP forecasts valid for one calendar year unless it is revised. After one year we assume that TP recommendation expires.

Consistent with other studies on analysts' expected returns that work with price targets ~\citep{bradshaw2002,brav2003,da2011}, we truncate the sample of $TP/P-1$ at the \Sexpr{ trunk.percent *100}\textsuperscript{th} percentile (values below \Sexpr{q.data[,min(trunk.view)]}) and at the \Sexpr{(1-trunk.percent)*100}\textsuperscript{th} percentile (values above \Sexpr{ q.data[,max(trunk.view)]}). This is done due to occurrence of the extreme values. Most of these extreme values are driven by misalignment errors found on I/B/E/S data\footnote{We found some differences between the  DataStream and I/B/E/S the databases. In some cases the stock-splits and the dividends were not properly adjusted.}. To implement ranking, we require that a stock had at least three analysts per quarter and that an analyst has to be active in covering a particular stock for at least 3 years (12 quarters). After all the  data requirements, our final sample number of analysts issued target prices is \Sexpr{core.dt[,.N,by=.(Broker,type)][,.N,by=type][,N][2]} covering \Sexpr{core.dt[,.N,by=.(Stock,type)][,.N,by=type][,N][2]} S\&P500 stocks. Overall, the number of observations ($\mathrm{Stock} \times \mathrm{Analyst} \times  \mathrm{Quarter}$) is reduced  from \Sexpr{ prettyNum(q.data[,.N,by=type][,N][2], big.mark=' ')} (initial) to \Sexpr{ prettyNum(core.dt[,.N,by=type][,N][2],big.mark=' ')} (filtered).

In the case of EPS forecasts, the initial file of quarterly EPS forecast consists of \Sexpr{eps.dt[,.N,by=.(Broker)][,.N]} analysts covering \Sexpr{eps.dt[,.N,by=Stock][,.N]} stocks. Considering the ranking data requirement, our final sample of EPS forecasts consist of  \Sexpr{complete.dt[,.N,by=.(Broker,type)][,.N,by=type][,N][2]} analysts covering \Sexpr{complete.dt[,.N,by=.(Stock,type)][,.N,by=type][,N][2]} S\&P500 stocks. The total number of observation is \Sexpr{prettyNum(complete.dt[,.N,by=type][,N][2],big.mar=' ')}.
...
We apply the ranking procedure outline in \vref{def-q} to the two datasets. For target price rankings, we use the average daily errors within one quarter as the measure of analysts' forecasting ability (\vref*{tp:pmafe}).

\vref{tab:example} and \vref{fig:example} illustrate an example illustrate how we estimate the \textit{PMAFE}. Four analysts had valid target prices for Amazon for second quarter of 1999. We plot the daily Amazon price against the analysts' target prices. \ref{tab:example} shows the resulting TP and EPS rankings. On the bases of the average daily errors, LEGG is the most accurate in forecasting stock price and  DLJ is the least accurate. For the EPS case (panel B), PACCREST is the most accurate in EPS forecasting and RBRTSON is the least.

\begin{table}
  \caption{Example of ranking}
  \label{tab:example}
\ This table shows target prices (panel A) and EPS forecasts (panel B) rankings for Amazon (AMZN) for the second quarter of 1999. We apply (\vref*{tp:rank}) to obtain the ranks of the analysts. \emph{TP} are target prices; $\overline{PMAFE}^{TP}$ is the daily average proportional mean adjusted TP error. For the EPS case, $PRED$ are the EPS forecasts issued by the analysts; $PMAFE^{EPS}$ is the proportional mean-adjusted forecast error of quarterly EPS forecasts.
...
print(xtable(setnames(q.data.ex[Stock=='AMZN'&q.id=='1999 Q2'][,list('Analyst'=Broker,'\\emph{TP}'=PT,'$\\overline{PMAFE}^{TP}$'=round(score,3),rank=rank(score))][order(rank)],4,'$rank^{TP}$'),display=c('s','d','f','f','f')),include.rownames=F,only.contents = T,hline.after=NULL,add.to.row=list(pos=list(0),command=c('\\midrule \n')),sanitize.text.function = function(x) x)
@
\midrule
\end{tabularx}
\begin{tabularx}{\linewidth}{r*{4}{Y}}
...
print(xtable(setnames(ex.eps[Stock=='AMZN'&q.id=='1999 Q2'][,list('Analyst'=Broker,'$PRED$'=Est,'$PMAFE^{EPS}$'=eps.pmafe,rank)][order(rank)],4,'$rank^{EPS}$'),display=c('s','d','f','f','f'),digits=3),include.rownames=F,only.contents=T,hline.after=NULL,add.to.row=list(pos=list(0),command=c('\\midrule \n')),sanitize.text.function = function(x) x)
@
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}
<<example-fig,echo=F,include=TRUE,results='hide',fig.width=10.7,fig.height=8.3>>=

example.dt$Broker <- factor(example.dt$Broker,levels=unique(example.dt$Broker))
test <- rbind(example.dt[,.(s.Date,PT,Series=Broker)][,Broker:='Analyst'],example.dt[,.(s.Date,PT=priceAdj)][,Broker:='AMZN'][,Series:='AMZN'])
test$Series <- factor(test$Series,levels=c('AMZN',as.character(unique(example.dt$Broker))))


ggplot(test,aes(x=s.Date,y=PT,group=Series,color=Series))+theme_bw()+ylab('Dollars')+ggtitle('Daily stock price (AMZN) compared to analyst\'s target prices \n over the period of the second calendar quarter of 1999')+geom_line()+theme(axis.title.x=element_blank(),text=element_text(size=20,family='Times'),legend.title=element_blank())
@
\caption{Amazon daily stock price and analysts target prices}
\label{fig:example}
\ Target price and actual prices for Amazon  the second quarter of 1999.
\end{figure}


%The daily frequency allows us to capture the analysts' performance with better precision. Panel A of table (\vref{tab:example}) shows an example of rankings based on target prices. As we observe, during the second quarter of 1999, four analysts have valid target prices for Amazon (AMZN) stock. At the end of the quarter, we calculate the average daily error for each of the analysts and apply equations (\vref{tp:pmafe}) and (\vref{tp:rank}) to obtain the ranks of each of the analysts. Figure (\vref{fig:example}) demonstrates the daily AMZN stock price and the levels of the analysts' target prices. The model gives to analysts the follow ranking: LEGG(1), MONTSEC(2), KAUFBRO(3), DLJ(4). In this example, LEGG is the most accurate in forecasting the AMZN stock and DLJ is the least.
...
\vref{tab:rank-stat} shows a contingency analysis of the ranks.
...
\vref{tab:view-stat} presents the descriptive statistics of the analysts' expected returns conditional on the different information sets.
...
Panel A of \vref{tab:view-stat} show the statistics for the consensus expectations as defined in \vref{consq}. As mentioned above in \vref{inf-set}, the consensus views have equal weights among the analysts, regardless of their ranks; thus, for the case of \tr{}, \naive{}, and \default{}, the mean is the same regardless of knowing or not the present or past rankings ($Q_{cons}$ in  \vref{consq}). As such, the mean, median, and standard deviation in the \tr{}, \naive{}, and \default{} information sets are the same. For the sample of \all{} stocks the mean expected return is \Sexpr{test.a['Total',,'CONS','mean','all'][[2]] }\%. However, since views also include the confidence (\vref*{eq-cv}), which is based on analysts past performance, the results of the trading strategy based on consensus expectations will be different for the \naive{} and \default{} information sets.

%and for the \same{} sample of stocks the mean is \Sexpr{test.a['Total',,'CONS','mean','same'][[2]]*100 }\%.

Panel B of the table shows the TP accuracy weighted average expected returns. For the sample of \all{} stocks the weighted average returns for \tr{}, \naive{}, and \default{} information sets are respectively \Sexpr{test.a['Total',,'PT','mean','all'][[1]] }\%, \Sexpr{test.a['Total',,'PT','mean','all'][[2]]}\%, and \Sexpr{test.a['Total',,'PT','mean','all'][[3]]}\%.
...
Panel C shows the EPS based weighted expected return. The average return for the \tr{}, \naive{}, and \default{} information sets are respectively \Sexpr{test.a['Total',,'EPS','mean','all'][[1]] }\%, \Sexpr{test.a['Total',,'EPS','mean','all'][[2]]}\%, and \Sexpr{test.a['Total',,'EPS','mean','all'][[3]]}\%.
...
Table \vref{tab:stocks} shows the number of active stocks for each of the trading strategies (\textit{CONS}, \textit{TP}, and \textit{EPS}) conditional on considered information sets.
...
We report the results from different trading strategies in Table \vref{tab:strategy}. We split the table into four panels. Panel A shows  the performance for  the passive (market) strategy \textit{Market}. Panels B to D compare the consensus , the TP rank weighted and the EPS rank weighted trading strategies for each of the information availability sets.
...
print(xtable(t(data.table('\\textit{Market}'=results.final)),display=c('s','f','f','f','d','f'),digits=3),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0),command=c('\\multicolumn{5}{l}{\\textbf{Panel A}} \\\\ \n')),sanitize.text.function = function(x) x)
cat('\\midrule \n')
@
\end{tabularx}
...
print(xtable(acast(melt(bl.results[1:3,1,,]),Var1~Var2+Var3,value.var='value'),display=c('s','f','f','f','f','f','f','d','d','f','f'),digits=3),only.contents=T,include.rownames=F,NA.string='-',include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(bl.results[1:3,1,,'all']),0,1,2),command=c('\\midrule \n','\\textit{CONS}&','\\textit{TP}&','\\textit{EPS}&')),sanitize.text.function = function(x) x)
@
\end{tabular}}
...
print(xtable(acast(melt(bl.results[1:3,2,,]),Var1~Var2+Var3,value.var='value'),display=c('s','f','f','f','f','f','f','d','d','f','f'),digits=3),only.contents=T,include.rownames=F,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(bl.results[1:3,2,,'all']),0,1,2),command=c('\\midrule \n','\\textit{CONS}&','\\textit{TP}&','\\textit{EPS}&')),sanitize.text.function = function(x) x)
@
\end{tabular}}
...
print(xtable(acast(melt(bl.results[1:3,3,,]),Var1~Var2+Var3,value.var='value'),display=c('s','f','f','f','f','f','f','d','d','f','f'),digits=3),only.contents=T,include.rownames=F,include.colnames=FALSE,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(0,1,2),command=c('\\textit{CONS}&','\\textit{TP}&','\\textit{EPS}&')),sanitize.text.function = function(x) x)
@
\bottomrule
\end{tabular}}
\end{table}
...
The analysis of the sub-periods performance of the \textit{TP} and \textit{EPS} -based strategies depicted in \vref{tab:substrategy} shows that the latter outperforms the former in terms of the annualized cumulative return only for the first two periods: in  2000Q1/2004Q4  \Sexpr{periods.array[1,'EPS',3,2,1,1]}\% vs. \Sexpr{periods.array[1,'TP',3,2,1,1]}\%  in 2001Q1/2005Q4 \Sexpr{periods.array[2,'EPS',3,2,1,1]}\%  vs. \Sexpr{periods.array[2,'TP',3,2,1,1]}\% respectively for the \textit{EPS} and the \textit{TP} strategies. For the all the other sup-periods, the annualized cumulative returns of the \textit{EPS} strategy are lower than those of the \textit{TP} strategy.
...
\vref{tab:substrategy-same} shows the sub-period results when we consider only the sample of stocks that integrates the TP and EPS dataset. In this setting (\same{}), the \textit{TP} strategy dominates the \textit{EPS} and \text{CONS} strategies in every sub-period.
