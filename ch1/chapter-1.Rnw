...
Panel C of table (\ref{tab:strategy}) shows the performance of the different smart strategies in the \naive{} information set. Strategies based on the target price accuracy outperform the \textit{Market} (\Sexpr{bl.results['Market',2,1]}\%) and show positive cumulative annualized returns of \Sexpr{bl.results['TP',2,1]}\%,  \Sexpr{bl.results['CONS',2,1]}\%,  respectively for the \textit{TP}, the \textit{CONS} whereas the \textit{EPS} strategy failed to achieve positive annualized cumulative return (\Sexpr{bl.results['EPS',2,1]}\%). Moreover, not only did the \textit{TP} strategy demonstrated its outperformance in annualized cumulative return outcome but also it dominates when we consider the risk-adjusted returns. The Sharpe Ratio for this strategy is \Sexpr{ bl.results['TP',2,'ann.sr']} which is higher than that Sharpe Ratios of the \textit{CONS} and  the \textit{EPS} strategies  (\Sexpr{ bl.results['CONS',2,'ann.sr']} and \Sexpr{bl.results['EPS',2,'ann.sr']} respectively). The results in panel B of table \ref{tab:substrategy} show as well that this strategy outperform the others for all of 5 trading sub-periods.


Panel D of table \ref{tab:strategy} demonstrates an interesting results in terms of annualized cumulative return: the smart estimate trading strategies showed a nearly equal performance (\Sexpr{bl.results['TP',3,1]}\%, \Sexpr{bl.results['EPS',3,1]}\%
for the \textit{TP}, \textit{EPS} respectively) while the \textit{CONS} was slightly less (\Sexpr{bl.results['CONS',3,1]}\%) but having all of them outperform the benchmark (\Sexpr{bl.results['Market',2,1]}\%). This suggests that
