...
sets <- c('all','same')
setwd('~/Documents/PhD/Projects/BL-strategies')
library(reshape2)
library(descr)
library(data.table)
library(ggplot2)
...
load('~/Documents/PhD/Projects/EPS/cache/complete.dt.RData')
...
load('~/Documents/PhD/Projects/EPS/cache/eps.dt.RData')
...
#fried_test <- friedman.test(ann.cum.ret ~ Views | Method, data = cum.ret[type=='same'][,.(Views,Method,ann.cum.ret)])

detach("package:rapportools", unload=TRUE)
detach("package:reshape", unload=TRUE)

##Dunn test
strat <- paste0('\\textit{',cum.ret[,levels(Views)],'}')
dun_t <- rbindlist(lapply(strategies,function(s){
  ldata<- lapply(dlply(cum.ret[Method==s],.(type),acast,Quarters~Views,value.var='ann.cum.ret'),alply,2,.dims=T)
  dtest <- dunn.test(ldata[[2]],table=F,kw=F)
  data.table(dtest$comparison,dtest$Z,dtest$P)[,Method:=s]
}))[,pair:=ifelse(V1 == '1 - 2', paste(strat[1], strat[2], sep=' - '), ifelse(V1 == '1 - 3', paste(strat[1], strat[3], sep=' - '), ifelse(V1=='2 - 3', paste(strat[2], strat[3], sep=' - '), ifelse( V1 == '1 - 4',  paste(strat[1], strat[4], sep=' - '), ifelse( V1 == '2 - 4',  paste(strat[2], strat[4], sep=' - '),  paste(strat[3], strat[4], sep=' - '))))))]
