...
res.dp.f <- function(stat.vvs,stat.type,meth)
  {rbind(stat.vvs[unlist(v.type)[1:3],meth,],'Total'=stat.type[1,meth,],
      stat.vvs[unlist(v.type)[4:9],meth,],'Total'=stat.type[2,meth,],
      stat.vvs[unlist(v.type)[10:13],meth,],'Total'=stat.type[3,meth,])
   }
...
v.type <- list(vvs.names[1:3],vvs.names[4:9],vvs.names[10:13])
...
metric.vvs$vvs.type <- factor(metric.vvs$vvs.type,levels=c('brok','stock','macro'))

metric.vvs$method <- factor(metric.vvs$method,levels=methods)
...
share.vvs <- acast(melt(na.omit(metric.vvs)[,list(NA_real_,mean(metric>0),sd(metric>0)),by=.(vvs,method)],id.vars=c('vvs','method')),vvs~method~variable)

share.type <- acast(melt(na.omit(metric.vvs)[,list(NA_real_,mean(metric>0),sd(metric>0)),by=.(vvs.type,method)],id.vars=c('vvs.type','method')),vvs.type~method~variable)

res.share <- abind(lapply(methods,function(i){res.dp.f(share.vvs,share.type,i)}),along=3,new.names=list(NULL,NULL,methods))

#res.dp.share <- abind(res.dp.tmp,res.share,along=4,new.names = list(NULL,NULL,NULL,c('dp','share')))

#res.dp <- acast(melt(res.dp.share),Var1~Var2~Var3~Var4)
...
Given the model, we apply \vref{ch3:eq-dp} to calculate a discriminative power of the variable. For an examle from \vpageref{ch3-tab01}, these values are \Sexpr{disc.power[1,dp]} and \Sexpr{format(round(disc.power[2,dp],4))} for variables $x_1$ and $x_2$ respectively. Based on this example, we conclude that the most discriminative variable is $x_2$. The result is intuitive.  Observe that variable $x_2$ takes only two values $a$ and $b$ with value $a$ repeating 4 times; thus, it has more probabilty to affect the rankings.
...
%\Vref{ch3-table:dp-ab} shows the results of identifying the discriminative power of the variables for different aggregation methods. Panel A shows the case with no aggregation (\raw{}) on state variables. The average discriminative power (in decreasing order) is \Sexpr{sort(res.dp[c(4,11,16),2,'raw'],decreasing = T)[1]} for Stock-specific variables (with \textit{\Sexpr{names(which.max(res.dp[v.type[[2]],2,'raw']))}} being the most contribuitive), \Sexpr{sort(res.dp[c(4,11,16),2,'raw'],decreasing = T)[2]} for Broker-specific variables (\textit{\Sexpr{names(which.max(res.dp[v.type[[1]],2,'raw']))}}), and \Sexpr{sort(res.dp[c(4,11,16),2,'raw'],decreasing = T)[3]} for macroeconomic variables (\textit{\Sexpr{names(which.max(res.dp[v.type[[3]],2,'raw']))}}).
...
%\ The table presents the discriminative power for cases when $DP>0$. Different types of aggregations are considered: \raw{} is the case of no aggregation, \diff{} is a first-differencing, \random{} is a random part of the time series decomposition, and \rollsd{} is the rolling 8 quarter standard deviation.\\

%\begin{landscape}
\begin{longtabu} to \linewidth{r*{6}{Y}}
\caption[Discriminate power of the variables, $DP>0$]{Discriminate power of the variables, $DP>0$.\\ \normalsize{The table presents the discriminative power for cases when $DP>0$. Different types of aggregations are considered: \raw{} is the case of no aggregation, \diff{} is a first-differencing, \random{} is a random part of the time series decomposition, and \rollsd{} is the rolling 8 quarter standard deviation.}}\\

\toprule
 Type&Variable&\raw{} &\diff& \random&\rollsd{}\\
\midrule
\endfirsthead
...
\midrule
\endhead
\multicolumn{6}{r}{... \textit{\footnotesize{continued on next page}}}\\
\endfoot
\endlastfoot
%\toprule
% Type&Variable&\raw{} &\diff& \random&\rollsd{} \\
%\midrule
%\endhead
%& &$DP$&share&$DP$&share&$DP$&share&$DP$&share\\
\multicolumn{6}{l}{\textbf{Panel A: Median}}\\
<<chap3-dp-median,echo=F,results='asis'>>=

print(xtable(data.table(vvs=rownames(res.dp[,1,]),res.dp[,1,])),display=c('s',rep('f',4)),only.contents=T,include.colnames=FALSE,include.rownames=F,hline.after=NULL,add.to.row=list(pos=list(c(4,11),0,4,11,c(1,2,5,6,7,8,9,12,13,14),c(3,10,15)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{4}{*}{Macro}& \n','&','\\cmidrule{2-6}& \n')))
@
\midrule
%\end{tabularx}
%\begin{tabularx}{\linewidth}{r*{6}{Y}}
\multicolumn{6}{l}{\textbf{Panel B: Mean}}\\
% Type&Variable& median & mean & st.dev\\
%  \midrule
<<chap3-dp-mean,echo=F,results='asis'>>=

print(xtable(data.table(vvs=rownames(res.dp[,2,]),res.dp[,2,])),display=c('s',rep('f',4)),only.contents=T,include.colnames=FALSE,include.rownames=F,hline.after=NULL,add.to.row=list(pos=list(c(4,11),0,4,11,c(1,2,5,6,7,8,9,12,13,14),c(3,10,15)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{4}{*}{Macro}& \n','&','\\cmidrule{2-6}& \n')))
@
\midrule
%\pagebreak
%\end{tabularx}
%\begin{tabularx}{\linewidth}{r*{6}{Y}}
\multicolumn{6}{l}{\textbf{Panel C: St. Dev}}\\
% Type&Variable& median & mean & st.dev\\
%  \midrule
<<chap3-dp-sd,echo=F,results='asis'>>=

print(xtable(data.table(vvs=rownames(res.dp[,3,]),res.dp[,3,])),display=c('s',rep('f',4)),only.contents=T,include.colnames=FALSE,include.rownames=F,hline.after=NULL,add.to.row=list(pos=list(c(4,11),0,4,11,c(1,2,5,6,7,8,9,12,13,14),c(3,10,15)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{4}{*}{Macro}& \n','&','\\cmidrule{2-6}& \n')))
@
\bottomrule
...
%'
%' \begin{table}
%' \caption{Discriminate power of the variables (continuted), $DP>0$}
...
%'
%' print(xtable(res.dp[,,'random']),display=c('s',rep('f',4)),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(c(4,11),0,4,11,c(1,2,5,6,7,8,9,12,13,14),c(3,10,15)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{4}{*}{Macro}& \n','&','\\cline{2-5}& \n')))
%' @
%' \midrule
%' \end{tabularx}
%' \begin{tabularx}{\linewidth}{r*{5}{Y}}
...
%'
%' print(xtable(res.dp[,,'roll.sd']),display=c('s',rep('f',4)),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(c(4,11),0,4,11,c(1,2,5,6,7,8,9,12,13,14),c(3,10,15)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{4}{*}{Macro}& \n','&','\\cline{2-5}& \n')))
%' @
%' \bottomrule
%' \end{tabularx}
%' \end{table}
...
ggplot(plot.data,aes(x=as.Date(q.id),y=(V1),fill=vvs.type))+geom_bar(stat='identity',position='fill')+theme_bw()+facet_grid(method~.,scales='free_y')+ggtitle('Average per quarter DP across stocks')+ylab('Mean DP')+xlab('Quarters')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),strip.text.y = element_text(angle = 0),text=element_text(size=20,family='Times'))
@
