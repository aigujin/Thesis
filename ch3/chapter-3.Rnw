...
data.to.display <- c('Variable','method','nbr.val','mean', 'median','std.dev')
desc.ind <- setnames(metric.vvs[,list(length(which(!is.na(value))),mean(value,na.rm=T),median(value,na.rm=T),sd(value,na.rm=T)),by=.(vvs,method)],data.to.display)
...
sample.stat.eps <- rbind(setnames(complete.dt[order(Sector),c(lapply(.SD,function(i){length(unique(i))}),.N),by=.(Sector),.SDcols=c('Stock','Broker')],4,'Forecast'),setnames(cbind('Total',complete.dt[,c(lapply(.SD,function(i){length(unique(i))}),Forecast=.N),.SDcols=c('Stock','Broker')]),1,'Sector'))
...
num.f.dt <- rbind(eps.dt[,.N,by=.(q.id,Sector,Broker,Stock)][,data.type:='sample'],complete.dt[,.N,by=.(q.id,Sector,Broker,Stock)][,data.type:='filter'])
...
per.broker <- setnames(cbind(
        ...
        rbind(num.f.dt[data.type=='sample',.N,by=.(q.id,Sector,Broker)][,mean(N),by=.(Sector)],list('Total',num.f.dt[data.type=='sample',.N,by=.(q.id,Sector,Broker)][,mean(N)]))[,V1],
        rbind(num.f.dt[data.type=='sample',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Broker)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='sample',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Broker)][,mean(V1)]))[,V1],
        rbind(num.f.dt[data.type=='sample',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Broker)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='sample',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Broker)][,mean(V1)]))[,V1]),c('Sector','Forecast','Stocks','Forecast/stock','Follow time,q'))


filter.per.broker <- setnames(cbind(
        rbind(complete.dt[,.N,by=.(q.id,Sector,Broker)][,mean(N),by=Sector],list('Total',complete.dt[,.N,by=.(q.id,Sector,Broker)][,mean(N)])),
        rbind(num.f.dt[data.type=='filter',.N,by=.(q.id,Sector,Broker)][,mean(N),by=.(Sector)],list('Total',num.f.dt[data.type=='filter',.N,by=.(q.id,Sector,Broker)][,mean(N)]))[,V1],
        rbind(num.f.dt[data.type=='filter',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Broker)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='filter',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Broker)][,mean(V1)]))[,V1],
        rbind(num.f.dt[data.type=='filter',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Broker)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='filter',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Broker)][,mean(V1)]))[,V1]),c('Sector','Forecast','Stocks','Forecast/stock','Follow time,q'))



###per stock
filter.per.stock <- setnames(cbind(
        rbind(complete.dt[,.N,by=.(q.id,Sector,Stock)][,mean(N),by=Sector],list('Total',complete.dt[,.N,by=.(q.id,Sector,Stock)][,mean(N)])),
        rbind(num.f.dt[data.type=='filter',.N,by=.(q.id,Sector,Stock)][,mean(N),by=.(Sector)],list('Total',num.f.dt[data.type=='filter',.N,by=.(q.id,Sector,Stock)][,mean(N)]))[,V1],
        rbind(num.f.dt[data.type=='filter',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Stock)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='filter',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Stock)][,mean(V1)]))[,V1],
        rbind(num.f.dt[data.type=='filter',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Stock)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='filter',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Stock)][,mean(V1)]))[,V1]),c('Sector','Forecast','Broker','Forecast/broker','Follow time,q'))
...
per.stock <- setnames(cbind(
        ...
        rbind(num.f.dt[data.type=='sample',.N,by=.(q.id,Sector,Stock)][,mean(N),by=.(Sector)],list('Total',num.f.dt[data.type=='sample',.N,by=.(q.id,Sector,Stock)][,mean(N)]))[,V1],
        rbind(num.f.dt[data.type=='sample',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Stock)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='sample',mean(N),by=.(Sector,Stock,Broker)][,mean(V1),by=.(Sector,Stock)][,mean(V1)]))[,V1],
        rbind(num.f.dt[data.type=='sample',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Stock)][,mean(V1),by=Sector],list('Total',num.f.dt[data.type=='sample',.N,by=.(Sector,Broker,Stock)][,mean(N),by=.(Sector,Stock)][,mean(V1)]))[,V1]),c('Sector','Forecast','Broker','Forecast/broker','Follow time,q'))

metric.vvs$vvs.type <- factor(metric.vvs$vvs.type,levels=c('brok','stock','sector','macro'))
#metric.m <- acast(melt(na.omit(metric.vvs)[,list(mean(metric),median(metric),sd(metric)),by=.(vvs.type,method)]),vvs.type~method+variable,value.var='value')
#test <- setkey(na.omit(metric.vvs)[,list(mean(metric),median(metric),sd(metric)),by=.(vvs,method)],vvs)

stat.vvs <- acast(melt(na.omit(metric.vvs)[,metric:=round(metric,3)][metric!=0,list(median(metric),mean(metric),sd(metric)),by=.(vvs,method)],id.vars=c('vvs','method')),vvs~method+variable)

stat.type <- acast(melt(na.omit(metric.vvs)[,metric:=round(metric,3)][metric!=0,list(median(metric),mean(metric),sd(metric)),by=.(vvs.type,method)],id.vars=c('vvs.type','method')),vvs.type~method+variable)

#test <- acast(melt(stat.vvs,id.vars=c('method','vvs')),vvs+variable~method)



#na.omit(metric.vvs)[,mean(metric),by=.(vvs.type,method)]
...
cat("Type&Variable & Observ & Median & Mean & std.dev\\\\\n")
cat('\\midrule \n')
#cat(rep('&',ncol(results.final)),'\\\\')
print(xtable(m.ind.vvs[unlist(v.type),'raw',c(1,3,2,4)],display=c('d','d','f','f','f'),digits=3),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(c(3,8,9),0,3,8,9,c(1,2,4,5,6,7,10,11,12)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{1}{*}{Sector}& \n','\\multirow{4}{*}{Macro}& \n','&')))
@
\bottomrule
\end{tabularx}
\end{center}
...
\begin{table}
\caption{Averages of forecasts and stocks per broker}
...
\begin{tabularx}{\linewidth}{r*{5}{Y}}
 \toprule
\multicolumn{4}{c}{\textbf{Panel A: Sample data}}\\
  \cline{2-5}
...
\end{tabularx}
\begin{tabularx}{\linewidth}{r*{5}{Y}}
\multicolumn{4}{c}{\textbf{Panel B: Filtered data}}\\
%  \cline{2-5}
% & Forecasts & Brokers & Forecast/stock & follow time, q \\
 \midrule
<<chap3-per-brok-filter,echo=F,results='asis'>>=
print(xtable(filter.per.broker,display=c('s','d','f','f','f','f'),align=c('r',rep('c',ncol(per.stock)))),only.contents=T,include.colnames=FALSE,include.rownames=F,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(per.stock)-1),command=c('\\midrule \n')))
@
\bottomrule
\end{tabularx}
...
\begin{table}
\caption{Averages of forecasts and brokers per stock}
\ The table presents the averages per quarter statistics based on the per Broker (Panel A) and per Stock (Panel B) perspective. Namely, the columns show the average number of forecast per quarter per Broker (per Stock), average number of Stocks (Brokers) per quarter, average number of forecasts per Stock (Broker) per quarter, and average number of quarters  a broker follows a stock (a stock being followed by a broker).
\begin{center}
%\begin{tabular}{rcccc}
\begin{tabularx}{\linewidth}{r*{5}{Y}}
...
print(xtable(per.stock,display=c('s','d','f','f','f','f'),align=c('r',rep('c',ncol(per.broker)))),only.contents=T,include.colnames=FALSE,include.rownames=F,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(per.broker)-1),command=c('\\midrule \n')))
@
\midrule
\end{tabularx}
\begin{tabularx}{\linewidth}{r*{5}{Y}}
\multicolumn{4}{c}{\textbf{Panel B: Filter data}}\\
%  \cline{2-5}
% & Forecasts & Brokers & Forecast/broker & follow time, q \\
 \midrule
<<chap3-per-stock-filter,echo=F,results='asis'>>=
print(xtable(filter.per.stock,display=c('s','d','f','f','f','f'),align=c('r',rep('c',ncol(per.stock)))),only.contents=T,include.colnames=FALSE,include.rownames=F,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(nrow(per.stock)-1),command=c('\\midrule \n')))
@
\bottomrule
\end{tabularx}
\label{ch3-table:forecasts-broker}
...
\toprule
Type&Variable&\multicolumn{3}{c}{\diff}&\multicolumn{3}{c}{\random}&\multicolumn{3}{c}{\raw}&\multicolumn{3}{c}{\rollsd}\\
\cline{3-14}
 && median & mean & st.dev&median & mean & st.dev&median & mean & st.dev&median & mean & st.dev \\
  \midrule
<<chap3-dp-raw,echo=F,results='asis'>>=

res.dp <- rbind(stat.vvs[unlist(v.type)[1:3],],'Broker Total'=stat.type[1,],
      stat.vvs[unlist(v.type)[4:8],],'Stock total'=stat.type[2,],
      'sec.ret'=stat.vvs[unlist(v.type)[9],],
      stat.vvs[unlist(v.type)[10:13],],'Macro total'=stat.type[4,])
print(xtable(res.dp),display=c('s',rep('f',12)),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(c(4,10,11),0,4,10,11,c(1,2,5,6,7,8,12,13,14),c(3,9,15)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{1}{*}{Sector}& \n','\\multirow{4}{*}{Macro}& \n','&','\\cline{2-14}& \n')))
@
\bottomrule
\end{tabular}}
\label{ch3-table:dp}
...
%'       'sec.ret'=stat.vvs[unlist(v.type)[9],],
%'       stat.vvs[unlist(v.type)[10:13],],'Macro total'=stat.type[4,])
%' print(xtable(res.dp),display=c('s',rep('f',4)),only.contents=T,include.colnames=FALSE,include.rownames=T,hline.after=NULL, format.args=list(big.mark = " ", decimal.mark = "."),add.to.row=list(pos=list(c(4,10,11),0,4,10,11,c(1,2,5,6,7,8,12,13,14),c(3,9,15)),command=c('\\midrule \n','\\multirow{3}{*}{Broker}& \n','\\multirow{5}{*}{Stock}& \n','\\multirow{1}{*}{Sector}& \n','\\multirow{4}{*}{Macro}& \n','&','\\cline{2-5}& \n')))
%' @
%' \bottomrule
%' \end{tabularx}
%' \label{ch3-table:dp}
...
<<chap3-fig-mean-f,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.align='center',fig.cap='Average number of forecast per broker.',fig.pos='hp'>>=
...
ggplot(num.f.dt[,mean(N),by=.(q.id,Sector,data.type)],aes(x=as.Date(q.id),y=V1,color=data.type))+theme_bw()+geom_smooth(method='loess',se=F)+xlab('Quarters')+ylab('Mean forecast')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),text=element_text(size=20,family='Times'))+facet_wrap(~Sector,ncol=2)+geom_line()
@

<<chap3-fig-num,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Total number of forecasts'>>=
...
ggplot(num.f.dt[,.N,by=.(q.id,Sector,data.type)],aes(x=as.Date(q.id),y=N,color=data.type))+theme_bw()+geom_smooth(method='loess',se=F)+xlab('Years')+ylab('Forecasts')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),text=element_text(size=20,family='Times'))+facet_wrap(~Sector,ncol=2,scale='free_y')
@

<<chap3-fig-stocks-brok,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Average number of stocks per broker'>>=
#ggplot(num.f.dt[,.N,by=.(q.id,Sector,Broker)][,mean(N),by=.(q.id,Sector)],aes(x=as.Date(q.id),y=V1))+geom_line()+facet_wrap(~Sector,ncol=2,scale='free_x')+theme_bw()+geom_smooth(method='loess',se=F)+xlab('Years')+ylab('Forecasts')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),text=element_text(size=20,family='Times'))

ggplot(num.f.dt[,.N,by=.(q.id,Broker,Sector,data.type)][,mean(N),by=.(q.id,Sector,data.type)],aes(x=as.Date(q.id),y=V1,color=data.type))+theme_bw()+geom_smooth(method='loess',se=F)+xlab('Years')+ylab('Forecasts')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),text=element_text(size=20,family='Times'))+facet_wrap(~Sector,ncol=2,scales='free_y')+geom_line()
@

<<chap3-fig-brok-stocks,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Average number of brokers per stock'>>=
ggplot(num.f.dt[,.N,by=.(q.id,Sector,Stock,data.type)][,mean(N),by=.(q.id,Sector,data.type)],aes(x=as.Date(q.id),y=V1,color=data.type))+facet_wrap(~Sector,ncol=2,scale='free_y')+theme_bw()+geom_smooth(method='loess',se=F)+xlab('Years')+ylab('Forecasts')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),text=element_text(size=20,family='Times'))+geom_line()+geom_hline(yintercept=3)
...
<<chap3-fig-dp,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Discriminative power overtime'>>=

ggplot(na.omit(metric.vvs)[,mean(metric),by=.(vvs.type,method,q.id)],aes(x=as.Date(q.id),y=V1,fill=vvs.type))+geom_bar(stat='identity',position='stack')+theme_bw()+facet_grid(method~.,scales='free_y')+ggtitle('Average per quarter DP across stocks')+scale_y_continuous(labels=comma)+ylab('% of total')+xlab('Quarters')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),strip.text.y = element_text(angle = 0),text=element_text(size=20,family='Times'))

vvs.col <- c("#CA7272","#66B34C","#7370CE","#5FA5B7","#B89537","#CF52CF","#943A79","#D5552F","#535678","#4B7241","#C58CC1","#7F442A","#D63E6B")

ggplot(na.omit(metric.vvs)[,metric:=round(metric,2)][metric!=0],aes(x=metric,fill=vvs))+theme_bw()+facet_grid(method~.,scales='free_y')+ggtitle('Average per quarter DP across stocks')+scale_y_continuous(labels=comma)+ylab('% of total')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),strip.text.y = element_text(angle = 0),text=element_text(size=20,family='Times'))+geom_histogram()+scale_x_continuous(limits=c(0,1000))+scale_fill_manual(values=vvs.col)+guides(fill=guide_legend(nrow=2,byrow=TRUE))



ggplot(na.omit(metric.vvs)[,mean(metric),by=.(Sector,vvs.type,method)],aes(x=vvs.type,y=V1,group=vvs.type,fill=Sector))+geom_bar(stat='identity')+theme_bw()+facet_grid(method~.,scales='free_x')+ggtitle('Average DP across stocks')+scale_y_continuous(labels=comma)+ylab('average DP')+xlab('Variables')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),strip.text.y = element_text(angle = 0),text=element_text(size=20,family='Times'))+scale_fill_manual(values=vvs.col)+guides(fill=guide_legend(nrow=2,byrow=TRUE))

@

<<chap3-fig-dp-sector,echo=FALSE, include=TRUE,results='hide',fig.width=10.7,fig.height=8.3,fig.cap='Discriminative power overtime'>>=

ggplot(na.omit(metric.vvs)[,mean((metric)),by=.(vvs.type,method,q.id,Sector)],aes(x=as.Date(q.id),y=V1,fill=vvs.type))+geom_area(stat='identity',position='fill',color='black')+theme_bw()+facet_grid(Sector~method,scales='free_y')+ggtitle('Average per quarter DP across stocks')+scale_y_continuous(labels=percent,breaks=c(1))+ylab('% of total')+xlab('Quarters')+theme(plot.title = element_text(colour = "Black"),legend.position='top',legend.title=element_blank(),strip.text=element_text(size=20),strip.text.y = element_text(angle = 0),text=element_text(size=20,family='Times'))
@
% \begin{figure}[hp]
% \begin{center}
